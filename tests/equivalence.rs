//! The keystone equivalence witness: one declared Ethos unit, ingested through the
//! LEGACY front end (`schema-language` migration) and through the NATIVE front end
//! (`core-ethos` six-slot document decode), BOTH bound through the currently wired
//! legacy central identity authority (a live `sema-storage` runtime), yields IDENTICAL
//! `EncodedEthos` content identity.
//!
...
//! declared unit that received the same assignment build identical encoded content.
//!
...
//! `spirit-min` ITSELF ([`spirit_min_ingests_to_identical_ethos_identity`]), the real
...
//! ([`legacy_and_native_ingest_to_identical_ethos_identity`]) is KEPT: it guards the
//! canonicalisation independently of the specific `spirit-min` shape.

use core_ethos::{EncodedReference, EncodedType};
use ethos_engine::ParsedEthos;
...
const EQUIVALENCE_MIN: &str = include_str!("fixtures/equivalence-min.ethos");

/// Bind one parsed Ethos unit's declared identities against the authority under `whole`,
...
async fn bind(
    ...
    let declarations = parsed
        .declared_identities()
        .expect("declared identities from parsed Ethos");
    let reply = runtime
...
/// The real `spirit-min` Ethos, verbatim — the same fixture the socket-live
...
const SPIRIT_MIN: &str = include_str!("fixtures/spirit-min.ethos");

/// Ingest one source through BOTH front ends, bind each against the one authority under
/// the same frozen `SchemaWholeHandle`, and assert the two built universes carry identical
/// `EncodedEthos` content identity. This is the whole point of the authority-provided
/// universe path: two front ends that intern into disagreeing private name tables,
/// re-stamped through one assignment, collapse to identical encoded content.
async fn assert_front_ends_agree(source: &str, whole_key: &[u8]) {
    ...
    // The frozen central authority still calls this a schema-whole handle. Both front
    // ends present one handle, so it binds the same universe and identities to each.
    let whole = SchemaWholeHandle(whole_key.to_vec());

    let legacy = ParsedEthos::from_legacy(source).expect("legacy front end");
    let native = ParsedEthos::from_native(source).expect("native front end");
    ...
    assert_eq!(
        ...
        "one authority, two front ends: the same declared Ethos unit yields identical \
         EncodedEthos content identity",
    );
...
async fn legacy_and_native_ingest_to_identical_ethos_identity() {
...
/// front ends to identical `EncodedEthos` content identity, now that the native front end
/// has converged onto the 2026-07-17 rulings.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn spirit_min_ingests_to_identical_ethos_identity() {
...
fn assert_summary_is_a_newtype_over_description(parsed: &ParsedEthos, front_end: &str) {
...
/// The missing round-trip witness (review item on beads .36): the NATIVE `core-ethos`
...
fn native_decode_lowers_single_field_brace_summary_to_a_newtype() {
    let native = ParsedEthos::from_native(SPIRIT_MIN).expect("native front end decodes spirit-min");
    assert_summary_is_a_newtype_over_description(&native, "native");

    let legacy = ParsedEthos::from_legacy(SPIRIT_MIN).expect("legacy front end decodes spirit-min");
